...
In this project, we consider three datasets available in the Python module ‘Fairlearn’: fetch_acs_income(), fetch_diabetes_hospital(), and fetch_credit_card(). Data dictionaries are included as YAML-files under the following link: [URL](https://github.com/acintron-a/RAI_best-practices-6190/tree/main/project_setup)

Due to the dimensions of these datasets, I had a difficult time loading them, let alone saving them as CSV-files. A Jupyter notebook with commands for installing the Python module ‘Fairlearn’ and to access the datasets is included in the same directory linked above.
